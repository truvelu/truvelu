...
import { useCanvasStore, createCanvasFinder, CanvasFinder } from "@/zustand/canvas";
...
export function useCanvasExists(type: CanvasType, roomId: string, threadId: string) {
  const canvasMap = useCanvasStore((state) => state.canvasMap);
  const finder = createCanvasFinder(canvasMap);
...
export function MyComponent() {
  ...
  const contentCanvas = finder.findByTypeAndRoom(CanvasType.CONTENT, "room-123");
...
export function processExternalCanvasMap(externalCanvasMap: Map<string, Map<string, any>>) {
